...
begin
  ...
  begin
    ...
    assert decode_op_sel_word(x"01000000") = FPU_OP_MOVE
      report "decode_op_sel_word MC68881 FMOVE mapping failed." severity failure;
    assert decode_op_sel_word(x"01000004") = FPU_OP_SQRT
      report "decode_op_sel_word MC68881 FSQRT mapping failed." severity failure;
    assert decode_op_sel_word(x"01000041") = FPU_OP_FNOP
      report "decode_op_sel_word MC68881 FNOP mapping failed." severity failure;
    assert decode_op_sel_word(x"01000042") = FPU_OP_FSCC
      report "decode_op_sel_word MC68881 FScc mapping failed." severity failure;
    assert decode_op_sel_word(x"01000043") = FPU_OP_FBCC
      report "decode_op_sel_word MC68881 FBcc mapping failed." severity failure;
    assert decode_op_sel_word(x"01000044") = FPU_OP_FDBCC
      report "decode_op_sel_word MC68881 FDBcc mapping failed." severity failure;
    assert decode_op_sel_word(x"01000046") = FPU_OP_FSAVE
      report "decode_op_sel_word MC68881 FSAVE mapping failed." severity failure;
    assert decode_op_sel_word(x"01000047") = FPU_OP_FRESTORE
      report "decode_op_sel_word MC68881 FRESTORE mapping failed." severity failure;
    assert decode_op_sel_word(x"0000000D") = FPU_OP_SIN
      report "decode_op_sel_word legacy SIN mapping failed." severity failure;
    assert decode_op_sel_word(x"0100001D") = FPU_OP_COS
      report "decode_op_sel_word MC68881 FCOS mapping failed." severity failure;
    assert decode_op_sel_word(x"0100000F") = FPU_OP_TAN
      report "decode_op_sel_word MC68881 FTAN mapping failed." severity failure;
    assert decode_op_sel_word(x"01000030") = FPU_OP_SINCOS
      report "decode_op_sel_word MC68881 FSINCOS mapping failed." severity failure;
    assert decode_op_sel_word(x"0100001C") = FPU_OP_ACOS
      report "decode_op_sel_word MC68881 FACOS mapping failed." severity failure;
    assert decode_op_sel_word(x"0100000C") = FPU_OP_ASIN
      report "decode_op_sel_word MC68881 FASIN mapping failed." severity failure;
    assert decode_op_sel_word(x"0100000A") = FPU_OP_ATAN
      report "decode_op_sel_word MC68881 FATAN mapping failed." severity failure;
    assert decode_op_sel_word(x"0100000D") = FPU_OP_ATANH
      report "decode_op_sel_word MC68881 FATANH mapping failed." severity failure;
    assert decode_op_sel_word(x"01000019") = FPU_OP_COSH
      report "decode_op_sel_word MC68881 FCOSH mapping failed." severity failure;
    assert decode_op_sel_word(x"01000010") = FPU_OP_ETOX
      report "decode_op_sel_word MC68881 FETOX mapping failed." severity failure;
    assert decode_op_sel_word(x"01000008") = FPU_OP_ETOXM1
      report "decode_op_sel_word MC68881 FETOXM1 mapping failed." severity failure;
    assert decode_op_sel_word(x"01000014") = FPU_OP_LOGN
      report "decode_op_sel_word MC68881 FLOGN mapping failed." severity failure;
    assert decode_op_sel_word(x"01000006") = FPU_OP_LOGNP1
      report "decode_op_sel_word MC68881 FLOGNP1 mapping failed." severity failure;
    assert decode_op_sel_word(x"01000015") = FPU_OP_LOG10
      report "decode_op_sel_word MC68881 FLOG10 mapping failed." severity failure;
    assert decode_op_sel_word(x"01000016") = FPU_OP_LOG2
      report "decode_op_sel_word MC68881 FLOG2 mapping failed." severity failure;
    assert decode_op_sel_word(x"01000002") = FPU_OP_SINH
      report "decode_op_sel_word MC68881 FSINH mapping failed." severity failure;
    assert decode_op_sel_word(x"01000009") = FPU_OP_TANH
      report "decode_op_sel_word MC68881 FTANH mapping failed." severity failure;
    assert decode_op_sel_word(x"01000012") = FPU_OP_TENTOX
      report "decode_op_sel_word MC68881 FTENTOX mapping failed." severity failure;
    assert decode_op_sel_word(x"01000011") = FPU_OP_TWOTOX
      report "decode_op_sel_word MC68881 FTWOTOX mapping failed." severity failure;
    assert decode_op_sel_word(x"01000018") = FPU_OP_ABS
      report "decode_op_sel_word MC68881 FABS mapping failed." severity failure;
    assert decode_op_sel_word(x"0100001A") = FPU_OP_NEG
      report "decode_op_sel_word MC68881 FNEG mapping failed." severity failure;
    assert decode_op_sel_word(x"01000001") = FPU_OP_INT
      report "decode_op_sel_word MC68881 FINT mapping failed." severity failure;
    assert decode_op_sel_word(x"01000003") = FPU_OP_INTRZ
      report "decode_op_sel_word MC68881 FINTRZ mapping failed." severity failure;
    assert decode_op_sel_word(x"0100001E") = FPU_OP_GETEXP
      report "decode_op_sel_word MC68881 FGETEXP mapping failed." severity failure;
    assert decode_op_sel_word(x"0100001F") = FPU_OP_GETMAN
      report "decode_op_sel_word MC68881 FGETMAN mapping failed." severity failure;
    assert decode_op_sel_word(x"0100003A") = FPU_OP_TST
      report "decode_op_sel_word MC68881 FTST mapping failed." severity failure;
    assert decode_op_sel_word(x"7F000000") = FPU_OP_NOP
