...
/* HID usage -> ASCII (unshifted, shifted).  Covers printable keys
 * enough to drive the single-user tty.  Extend if you need more. */
static const uint8_t hid_to_ascii[HID_TABLE_SIZE][2] = {
    ...
    [0x27] = {'0',')'},
    [0x28] = {'\r','\r'}, [0x29] = {0x1B,0x1B}, /* Enter, Escape */
    [0x2A] = {0x7F,0x7F},                       /* Backspace -> DEL */
    [0x2B] = {'\t','\t'}, [0x2C] = {' ',' '},   /* Tab, Space */
    [0x2D] = {'-','_'},   [0x2E] = {'=','+'},
    [0x2F] = {'[','{'},   [0x30] = {']','}'},
    [0x31] = {'\\','|'},  [0x33] = {';',':'},
...
extern int next_scc_rx_push(uint8_t ch);
...
static void hid_push_key(uint8_t hid_key, uint8_t modifiers, int key_up)
{
    ...
    /* Also inject the ASCII equivalent into the SCC RX buffer for
     * the single-user tty / ROM monitor.  The hybrid console has
     * `mg_console_i = CONS_I_SCC_A`, so the shell reads from the
     * serial line; without this path, USB keypresses go to the KMS
     * queue (used by the window server) but never reach the tty. */
    if (!key_up) {
        int shift = (modifiers & (HID_MOD_LSHIFT | HID_MOD_RSHIFT)) != 0;
        uint8_t ch = hid_to_ascii[hid_key][shift];
        if (ch != 0) {
            if (modifiers & (HID_MOD_LCTRL | HID_MOD_RCTRL)) {
                /* Ctrl+letter: mask down to 0x01-0x1F (Ctrl+C -> 0x03 etc). */
                if (ch >= 'a' && ch <= 'z') ch -= 'a' - 1;
